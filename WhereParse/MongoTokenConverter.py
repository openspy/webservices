...
class MongoGreaterEqualsOperator(MongoToken):
    def __init__(self, lvalue, rvalue):
        ...
        self.rvalue = rvalue
    def GetMongoDocument(self):
        return {"$gte": [self.lvalue.GetMongoDocument(), self.rvalue.GetMongoDocument()]}
class MongoGreaterOperator(MongoToken):
    def __init__(self, lvalue, rvalue):
        self.lvalue = lvalue
        self.rvalue = rvalue
    def GetMongoDocument(self):
        return {"$gt": [self.lvalue.GetMongoDocument(), self.rvalue.GetMongoDocument()]}
class MongoLessEqualsOperator(MongoToken):
    def __init__(self, lvalue, rvalue):
        self.lvalue = lvalue
        self.rvalue = rvalue
    def GetMongoDocument(self):
        return {"$lte": [self.lvalue.GetMongoDocument(), self.rvalue.GetMongoDocument()]}
class MongoLessOperator(MongoToken):
    def __init__(self, lvalue, rvalue):
        self.lvalue = lvalue
        self.rvalue = rvalue
    def GetMongoDocument(self):
        return {"$lt": [self.lvalue.GetMongoDocument(), self.rvalue.GetMongoDocument()]}
